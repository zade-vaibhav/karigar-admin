...
export function KarigarPayments() {
    const [filteredKarigarOrders, setFilteredOrders] = useState([]);
    ...
    async function getLabor() {
        ...
        try {
            ...
            if (result.success === true) {
                ...
                const allOrders = result.labors.reduce((acc, labor) => {
                    return acc.concat(labor.order);
                }, []);
                setAllOrders(allOrders); // Store all orders
                setFilteredOrders(allOrders); // Set filtered orders to all initially
            }
        } catch (error) {
            console.error(error);
        }
...
export default KarigarPayments
